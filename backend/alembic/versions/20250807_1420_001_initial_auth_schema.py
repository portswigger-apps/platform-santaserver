...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_users_external_id", "users", ["external_id"], postgresql_where=sa.text("external_id IS NOT NULL")
    )
    op.create_index(
        "idx_users_provider_name", "users", ["provider_name"], postgresql_where=sa.text("provider_name IS NOT NULL")
    )
    op.create_index(
        "idx_users_locked_until", "users", ["locked_until"], postgresql_where=sa.text("locked_until IS NOT NULL")
    )
    op.create_index(
        "idx_users_password_expires",
        "users",
        ["password_expires_at"],
        postgresql_where=sa.text("password_expires_at IS NOT NULL"),
    )

    op.create_index("idx_groups_source_type", "groups", ["source_type"])
    op.create_index(
        "idx_groups_external_id", "groups", ["external_id"], postgresql_where=sa.text("external_id IS NOT NULL")
    )
    op.create_index(
        "idx_groups_provider_name", "groups", ["provider_name"], postgresql_where=sa.text("provider_name IS NOT NULL")
    )

    op.create_index("idx_user_sessions_user_id", "user_sessions", ["user_id"])
    op.create_index("idx_user_sessions_token_jti", "user_sessions", ["token_jti"])
    op.create_index(
        "idx_user_sessions_refresh_token_jti",
        "user_sessions",
        ["refresh_token_jti"],
    ...
    op.create_index(
        "idx_user_sessions_is_revoked", "user_sessions", ["is_revoked"], postgresql_where=sa.text("is_revoked = false")
    )
    ...
    op.create_index(
        "idx_auth_providers_enabled", "auth_providers", ["is_enabled"], postgresql_where=sa.text("is_enabled = true")
    )
