...
async def login(login_data: LoginRequest, request: Request, db: Session = Depends(get_db)):
    ...
    user = await auth_service.authenticate_user(
        username=login_data.username, password=login_data.password, ip_address=ip_address, user_agent=user_agent
    )
    ...
    token_pair = await auth_service.create_user_session(user=user, ip_address=ip_address, user_agent=user_agent)
...
async def refresh_token(refresh_data: RefreshRequest, request: Request, db: Session = Depends(get_db)):
    ...
    token_pair = await auth_service.refresh_token(
        refresh_token=refresh_data.refresh_token, ip_address=ip_address, user_agent=user_agent
    )
    ...
    return TokenResponse(
        access_token=token_pair.access_token, refresh_token=token_pair.refresh_token, token_type="bearer"
    )
...
async def logout(
    ...
    success = await auth_service.revoke_session(
        user=current_user, token=token, reason="user_logout", ip_address=ip_address, user_agent=user_agent
    )
...
async def logout_all_sessions(
    ...
    revoked_count = await auth_service.revoke_all_user_sessions(user=current_user, reason="user_logout_all")
...
async def update_current_user_profile(
    ...
    """
    allowed_fields = {"first_name", "last_name", "display_name", "department", "title", "phone"}
