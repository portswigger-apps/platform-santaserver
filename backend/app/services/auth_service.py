...
class AuthenticationService:
    ...
    async def authenticate_user(
        ...
        if not user:
            await self._log_security_event(
                None, "login_failed", {"reason": "user_not_found", "username": username},
                ip_address, user_agent, success=False, failure_reason="User not found"
            )
            return None
        ...
        if not user.is_active:
            await self._log_security_event(
                user.id, "login_failed", {"reason": "account_inactive"},
                ip_address, user_agent, success=False, failure_reason="Account inactive"
            )
            return None
        ...
        if user.locked_until and user.locked_until > datetime.now(timezone.utc):
            await self._log_security_event(
                user.id, "login_failed", {"reason": "account_locked"},
                ip_address, user_agent, success=False, failure_reason="Account locked"
            )
            return None
        ...
        if user.user_type == UserTypeEnum.LOCAL:
            if not user.password_hash:
                await self._log_security_event(
                    user.id, "login_failed", {"reason": "no_password_set"},
                    ip_address, user_agent, success=False, failure_reason="No password set"
                )
                return None
            ...
            if not SecurityUtils.verify_password(password, user.password_hash):
                ...
                if user.failed_login_attempts >= settings.MAX_LOGIN_ATTEMPTS:
                    ...
                    await self._log_security_event(
                        ...
                        ip_address, user_agent, success=False, failure_reason="Too many failed attempts"
                    )
                else:
                    await self._log_security_event(
                        user.id, "login_failed", 
                        {"reason": "invalid_password", "failed_attempts": user.failed_login_attempts},
                        ip_address, user_agent, success=False, failure_reason="Invalid password"
                    )
        ...
        await self._log_security_event(
            user.id, "login_successful", {"user_type": user.user_type.value},
    ...
    async def create_user_session(
        ...
        await self._log_security_event(
            user.id, "session_created", {"session_id": str(session.id)},
            ip_address, user_agent, success=True
        )
    ...
    async def refresh_token(
        ...
        if settings.REFRESH_TOKEN_ROTATION:
            ...
            session.refresh_expires_at = datetime.now(timezone.utc) + timedelta(days=settings.JWT_REFRESH_TOKEN_EXPIRE_DAYS)
        
        self.db.add(session)
        self.db.commit()
        
        await self._log_security_event(
            user.id, "token_refreshed", {"session_id": str(session.id)},
            ip_address, user_agent, success=True
        )
    ...
    async def change_password(
        ...
        if user.user_type == UserTypeEnum.LOCAL:
            if not user.password_hash or not SecurityUtils.verify_password(current_password, user.password_hash):
                await self._log_security_event(
                    user.id, "password_change_failed", {"reason": "invalid_current_password"},
                    ip_address, user_agent, success=False, failure_reason="Invalid current password"
                )
