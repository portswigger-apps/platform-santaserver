...
def require_permissions(resource: str, action: str):
    ...
    async def permission_dependency(
        user_permissions: Dict[str, List[str]] = Depends(get_user_permissions)
    ):
...
def require_role(role_name: str):
    ...
    async def role_dependency(
        ...
        user_role_stmt = select(Role).join(UserRole).where(
        ...
        group_role_stmt = select(Role).join(GroupRole).join(UserGroup).where(
